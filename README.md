...
מטרה נוספת היא יצירת כלי תכנון המספק ממשק נוח עם אפשרות לבחירת פרמטרים שונים. ממשק זה מאפשר לחוקר להריץ באופן יעיל את הסימולטור 
בתרחישים שונים של בחירת פרמטרים ולהסיק מסקנות בנוגע לתכנון מערכת מבצעית.

מטרות הפרויקט ומוטיבציה: מטרת הפרויקט היא פיתוח אלגוריתם לקידוד משולב מקור/ערוץ של חישה היפרספקטראלית. האלגוריתם נועד לבצע דחיסת תמונה היפרספקטראלית בשילוב קודים לתיקון שגיאות. 
האלגוריתם נועד לתמוך בלוויין המצויד במצלמה היפרספקטראלית המאפשרת צילום במספר אורכי גל לצורך זיהוי חתימות ספקטראליות של מזהמים באטמוספירה ועל הקרקע. 
רוחב הסרט המוגבל וזמן החליפה הקצר מעל תחנת הקרקע מחייבים דחיסת המידע בחלל לפני שידורו לכדור הארץ. 

מצד שני, דחיסת המידע מגדילה את רגישותו לשגיאות ועלולה לגרום לשגיאה קטסטרופאלית במהלך הפיענוח על הקרקע. לפיכך, נדרש מנגנון משולב של דחיסה ותיקון שגיאות, המאזן בין יעילות דחיסה לעמידות לשגיאות בערוץ תקשורת רועש.
כמו כן, מטרת הפרויקט היא לעמוד בדרישות הכמותיות שהוגדרו על ידי מנחה הפרויקט. הדרישות מבוססות על נתוני לוויינים היפרספקטראליים כפי שמתואר בספרות, וביצועי האלגוריתם נבחנו והודגמו על בסיס נתונים של לווייני חישה מרחוק. 
מטרה נוספת היא יצירת כלי תכנון המספק ממשק נוח עם אפשרות לבחירת פרמטרים שונים כגון: גודל התמונה, שיעור שגיאות (BER), ושימוש אופציונלי בקוד CRC. 
ממשק זה מאפשר לחוקר להריץ באופן יעיל את הסימולטור בתרחישים שונים של בחירת פרמטרים ולהסיק מסקנות בנוגע לתכנון מערכת מבצעית.

תמונות היפרספקטראליות מכילות מידע עשיר ושימושי בתחומים מגוונים ,כמו חקלאות, חקר סביבה ומדעי כדור הארץ. 
הצורך בלוויין זעיר למחקר אקלים וסביבה נובע מהעלייה במודעות הגלובלית לשינויי אקלים והשפעותיהם השליליות על הסביבה. 
לוויין המצויד במצלמה היפרספקטראלית יכול לספק נתונים מדויקים על ריכוזי מזהמים באטמוספירה ועל הקרקע, ובכך לתרום להבנת המצב הנוכחי ולפיתוח פתרונות יעילים. 
 ...
 **התקנת ספריות:**
   - התקן את כל הספריות הנדרשות לפרויקט מתוך קובץ: [requirements.txt](./requirements.txt)
     ```bash
